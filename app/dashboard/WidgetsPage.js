...
function ErrMsg({ msg }) {
  return (
    <div style={{ fontSize: 11, color: 'rgba(255,100,100,0.7)', marginTop: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
...
function DiscordWidget({ config }) {
  ...
  useEffect(() => {
    ...
    // Try WebSocket first for live updates, fall back to REST
    let heartbeat
    ...
    const restFallback = async (uid) => {
      try {
        ...
        else setErr('User not found — join discord.gg/lanyard first')
      } catch {
        setErr('Could not reach Lanyard API')
      }
      setLoad(false)
    }
  ...
  const listening = data?.activities?.find(a => a.type === 2) // Listening
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(88,101,242,0.2)', border: '1px solid rgba(88,101,242,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, color: '#5865F2', overflow: 'hidden' }}>
            {user?.avatar
              ? <img src={`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=64`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '50%' }} />
      ...
      {(spotify || listening) && (
        <div style={{ background: 'rgba(29,185,84,0.08)', border: '1px solid rgba(29,185,84,0.2)', borderRadius: 10, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
          {spotify?.album_art_url && (
            <img src={spotify.album_art_url} alt="" style={{ width: 32, height: 32, borderRadius: 6, flexShrink: 0 }} />
          )}
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 11, color: '#1DB954', fontWeight: 700, marginBottom: 1 }}>♪ Listening on Spotify</div>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{spotify?.song || listening?.details || '—'}</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{spotify?.artist || listening?.state || ''}</div>
          </div>
        </div>
      )}
        ...
        <div style={{ background: 'rgba(88,101,242,0.08)', border: '1px solid rgba(88,101,242,0.18)', borderRadius: 10, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
          {activity.assets?.large_image && (
            <img
              ...
              alt="" style={{ width: 32, height: 32, borderRadius: 6, flexShrink: 0 }}
              onError={e => e.target.style.display='none'}
            />
          )}
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 11, color: '#5865F2', fontWeight: 700, marginBottom: 1 }}>🎮 Playing</div>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{activity.name}</div>
            {activity.details && <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{activity.details}</div>}
          </div>
        </div>
      )}
    </div>
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setErr('User not found or API limit reached')
      }
      setLoad(false)
    })()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {[
          ...
          { label: 'Repos',     val: fmt(user.public_repos) },
        ].map(s => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          {repos.map(r => (
            <a key={r.id} href={r.html_url} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 8, textDecoration: 'none', transition: 'border-color .15s' }}
...
// ─── Weather (Open-Meteo + geocoding, completely free, no key) ─────────────────
const WMO_CODES = {
...
function WeatherWidget({ config }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // Weather
        const unit = isFahr ? 'fahrenheit' : 'celsius'
        const wx = await safeFetch(
          `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weathercode&temperature_unit=${unit}&wind_speed_unit=kmh&timezone=auto`
        )
        setData({ ...wx.current, city: name, country })
      } catch (e) {
        setErr(e.message || 'Could not load weather')
      }
      setLoad(false)
    })()
  ...
  const feel = Math.round(data.apparent_temperature)
  const unit = isFahr ? '°F' : '°C'

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 52, height: 52, borderRadius: 12, background: 'rgba(56,189,248,0.12)', border: '1px solid rgba(56,189,248,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>
          {icon}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: '#fff', lineHeight: 1 }}>{temp}{unit}</div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {[
          { label: 'Feels like', val: `${feel}${unit}` },
          { label: 'Humidity',   val: `${data.relative_humidity_2m}%` },
          { label: 'Wind',       val: `${Math.round(data.wind_speed_10m)} km/h` },
        ].map(s => (
...
function LastfmWidget({ config }) {
  ...
  useEffect(() => {
    ...
    const key = LASTFM_API_KEY || 'NO_KEY'
    if (!u) { setErr('No username configured'); setLoad(false); return }
    if (!LASTFM_API_KEY) { setErr('Add NEXT_PUBLIC_LASTFM_API_KEY to .env (free at last.fm/api)'); setLoad(false); return }
    ;(async () => {
      try {
        ...
        setErr(e.message || 'Could not load Last.fm data')
      }
      setLoad(false)
    })()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          {data.image?.[2]?.['#text']
            ? <img src={data.image[2]['#text']} alt="" style={{ width: 44, height: 44, borderRadius: '50%', border: '1px solid rgba(213,16,7,0.3)' }} />
            : <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(213,16,7,0.15)', border: '1px solid rgba(213,16,7,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🎵</div>}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {nowPlaying && (
        <div style={{ background: 'rgba(213,16,7,0.08)', border: '1px solid rgba(213,16,7,0.2)', borderRadius: 10, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
          {nowPlaying.image?.[1]?.['#text'] && (
            <img src={nowPlaying.image[1]['#text']} alt="" style={{ width: 36, height: 36, borderRadius: 6, flexShrink: 0 }} />
          )}
          <div style={{ minWidth: 0, flex: 1 }}>
      ...
      {tracks.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {tracks.filter(t => t['@attr']?.nowplaying !== 'true').slice(0, 2).map((t, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', background: 'rgba(255,255,255,0.02)', borderRadius: 8 }}>
              {t.image?.[1]?.['#text'] && <img src={t.image[1]['#text']} alt="" style={{ width: 28, height: 28, borderRadius: 4, flexShrink: 0 }} />}
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.75)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.name}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.artist?.['#text']}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // Avatar thumb (proxied via Roblox thumbnails API)
        let thumb = null
        try {
        ...
        setErr(e.message || 'Could not load Roblox data')
      }
      setLoad(false)
    })()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 48, height: 48, borderRadius: 12, background: 'rgba(224,0,0,0.12)', border: '1px solid rgba(224,0,0,0.25)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {data.thumb
            ? <img src={data.thumb} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            : <svg width="24" height="24" viewBox="0 0 24 24" fill="#e00000"><path d="M5.24 3L3 18.76 18.76 21 21 5.24 5.24 3zm9.12 10.9l-3.88-.6.6-3.88 3.88.6-.6 3.88z"/></svg>}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {[
          { label: 'Friends',   val: fmt(data.friendsCount) },
          { label: 'Followers', val: fmt(data.followersCount) },
        ].map(s => (
      ...
      {data.description && (
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
          {data.description}
        </div>
      )}
      <a href={`https://www.roblox.com/users/${data.id}/profile`} target="_blank" rel="noopener noreferrer"
        style={{ fontSize: 11, fontWeight: 600, color: '#e00000', background: 'rgba(224,0,0,0.08)', border: '1px solid rgba(224,0,0,0.2)', padding: '6px 12px', borderRadius: 8, textDecoration: 'none', alignSelf: 'flex-start', transition: 'all .15s' }}
        onMouseEnter={e => e.currentTarget.style.background='rgba(224,0,0,0.14)'}
        onMouseLeave={e => e.currentTarget.style.background='rgba(224,0,0,0.08)'}
      >View Profile ↗</a>
    </div>
  )
}

// ─── VALORANT (Henrik's API — free, no key required for basic) ─────────────────
const VAL_TIER_COLORS = {
  'Iron':'#a0a0a0','Bronze':'#8c5a28','Silver':'#b4b4b4','Gold':'#f0c040',
  'Platinum':'#40c8b4','Diamond':'#a060f0','Ascendant':'#40c860','Immortal':'#e04060','Radiant':'#f0e060',
}
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const mmr = await safeFetch(
          `https://api.henrikdev.xyz/valorant/v3/mmr/${reg}/${encodeURIComponent(name)}/${encodeURIComponent(tag)}`,
          { headers }
        )
        ...
        setErr(e.message?.includes('429') ? 'Rate limited — try again in a moment' : (e.message || 'Could not load VALORANT data'))
      }
      setLoad(false)
    })()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 52, height: 52, borderRadius: 12, background: `${tierColor}22`, border: `1px solid ${tierColor}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
          {current?.tier?.large_icon
            ? <img src={current.tier.large_icon} alt={tier} style={{ width: 40, height: 40, objectFit: 'contain' }} />
            : <svg width="26" height="26" viewBox="0 0 24 24" fill="none"><path d="M12 2L2 7l2 10 8 5 8-5 2-10L12 2z" stroke="#FF4655" strokeWidth="1.5"/><path d="M8 9l4 7 4-7" stroke="#FF4655" strokeWidth="1.5" strokeLinecap="round"/></svg>}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {peak && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: 'rgba(255,70,85,0.06)', border: '1px solid rgba(255,70,85,0.15)', borderRadius: 9 }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>
            Peak: <span style={{ fontWeight: 700, color: Object.entries(VAL_TIER_COLORS).find(([k]) => peak.tier?.name?.startsWith(k))?.[1] || '#FF4655' }}>{peak.tier?.name}</span>
            {peak.season && <span style={{ marginLeft: 6, color: 'rgba(255,255,255,0.3)' }}>({peak.season})</span>}
          </div>
        </div>
      )}
    </div>
...
// ─── Chess.com ─────────────────────────────────────────────────────────────────
const CHESS_EMOJI = { bullet:'⚡', blitz:'🔥', rapid:'⏱', daily:'📅', puzzle:'🧩' }
...
function ChessWidget({ config }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setErr(e.message || 'Could not load Chess.com data')
      }
      setLoad(false)
    })()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: '50%', overflow: 'hidden', border: '1px solid rgba(129,182,76,0.3)', flexShrink: 0 }}>
          {profile.avatar
            ? <img src={profile.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            : <div style={{ width: '100%', height: '100%', background: 'rgba(129,182,76,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>♟</div>}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {profile.title && <div style={{ fontSize: 10, fontWeight: 700, color: '#81b64c', background: 'rgba(129,182,76,0.12)', border: '1px solid rgba(129,182,76,0.25)', padding: '1px 6px', borderRadius: 4, display: 'inline-block', marginTop: 2 }}>{profile.title}</div>}
        </div>
          ...
          {modes.slice(0, 4).map(({ key, label }) => {
            ...
            return (
              <div key={key} style={{ background: 'rgba(129,182,76,0.06)', border: '1px solid rgba(129,182,76,0.14)', borderRadius: 8, padding: '7px 10px' }}>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginBottom: 3 }}>{CHESS_EMOJI[label.toLowerCase()]} {label}</div>
      ...
      {stats.tactics && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>
          🧩 Puzzle Rating: <span style={{ fontWeight: 700, color: '#81b64c' }}>{stats.tactics.highest?.rating || '—'}</span>
        </div>
      )}
    </div>
  )
...
// ─── TikTok (unofficial — uses a CORS-safe proxy) ─────────────────────────────
function TikTokWidget({ config }) {
  ...
  useEffect(() => {
    let u = config?.tiktokUsername?.trim().replace(/^@/, '')
    if (!u) { setErr('No username configured'); setLoad(false); return }
    ;(async () => {
      try {
        // Use tiktok-scraper-like endpoint via allorigins proxy
        const url = `https://www.tiktok.com/@${u}`
        const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(url)}`
        const raw = await safeFetch(proxyUrl)
        const html = raw.contents || ''

        // Parse __UNIVERSAL_DATA_FOR_REHYDRATION__ JSON from page
        const match = html.match(/"userInfo":\{"user":\{(.*?)\},"stats":\{(.*?)\}/)
        if (!match) throw new Error('Could not parse TikTok page — try a different proxy')

        // Alternative: use a simpler parse
        ...
        if (!statsMatch) throw new Error('Could not read stats — TikTok page structure may have changed')
        ...
        setData({
          ...
          avatar:    avatarMatch?.[1]?.replace(/\\\//g, '/') || null,
        })
      } catch (e) {
        setErr(e.message?.includes('parse') || e.message?.includes('read')
          ? 'TikTok blocked the request — try again or use a Next.js proxy route'
          : (e.message || 'Could not load TikTok data'))
      }
      setLoad(false)
    })()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,0,80,0.15)', border: '1px solid rgba(255,0,80,0.3)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
          {data.avatar
            ? <img src={data.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '50%' }} />
            : '🎵'}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {[
          { label: 'Followers', val: fmtBig(data.followers) },
          { label: 'Following', val: fmtBig(data.following) },
          { label: 'Likes',     val: fmtBig(data.likes) },
          { label: 'Videos',    val: fmt(data.videos) },
        ].map(s => (
...
function DiscordServerWidget({ config }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // Discord's public widget endpoint — server must have widget enabled
        const d = await safeFetch(`https://discord.com/api/guilds/${id}/widget.json`)
        if (d.code === 50004 || d.message) throw new Error('Enable the server widget in Discord Server Settings → Widget first')
        setData(d)
      } catch (e) {
        setErr(e.message || 'Could not load server data')
      }
      setLoad(false)
    })()
  }, [config?.serverId])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 48, height: 48, borderRadius: 14, background: 'rgba(88,101,242,0.15)', border: '1px solid rgba(88,101,242,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 20 }}>
          🏰
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {data.members?.length > 0 && (
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          {data.members.length > 8 && (
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginLeft: 2 }}>+{data.members.length - 8}</div>
          )}
        </div>
      )}
      ...
      {invite && (
        <a href={invite} target="_blank" rel="noopener noreferrer"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 12px', background: '#5865F2', borderRadius: 10, color: '#fff', fontSize: 12, fontWeight: 700, textDecoration: 'none', transition: 'opacity .15s' }}
...
function WidgetBadge({ color, children }) {
  return (
    <span style={{ fontSize: 9, fontWeight: 800, color, background: `${color}18`, border: `1px solid ${color}35`, padding: '3px 7px', borderRadius: 99, whiteSpace: 'nowrap', flexShrink: 0, letterSpacing: '0.04em' }}>
      {children}
    </span>
  )
}
...
function fmtBig(n) { if (n == null) return '—'; if (n >= 1e9) return (n/1e9).toFixed(1)+'B'; if (n >= 1e6) return (n/1e6).toFixed(1)+'M'; if (n >= 1e3) return (n/1e3).toFixed(1)+'K'; return String(n) }
...
const WIDGET_DEFS = [
  {
    ...
    color: '#5865F2',
    note: 'User must join discord.gg/lanyard first',
    ...
    fields: [{ key: 'discordId', label: 'Discord User ID', placeholder: '123456789012345678' }],
  ...
  },
  {
    ...
    fields: [{ key: 'githubUsername', label: 'GitHub Username', placeholder: 'username' }],
    Component: GitHubWidget,
  },
  {
    id: 'weather',
    name: 'Weather',
    desc: 'Current conditions — no API key needed',
    color: '#38bdf8',
  ...
  },
  {
    id: 'lastfm',
    name: 'Last.fm',
    desc: 'Now playing + scrobble count',
    color: '#d51007',
    note: 'Add NEXT_PUBLIC_LASTFM_API_KEY to .env (free at last.fm/api)',
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M10.84 17.42l-.9-2.45s-1.46 1.63-3.65 1.63c-1.94 0-3.32-1.69-3.32-4.38 0-3.45 1.74-4.69 3.44-4.69 2.45 0 3.22 1.59 3.87 3.65l.9 2.81c.9 2.73 2.58 4.93 7.44 4.93 3.48 0 5.83-1.07 5.83-3.87 0-2.26-1.29-3.44-3.71-4l-1.79-.39c-1.24-.28-1.6-.77-1.6-1.6 0-.94.74-1.49 1.96-1.49 1.32 0 2.03.49 2.14 1.68l2.74-.33C23.87 7.07 22.57 6 20.01 6c-2.38 0-4.89.9-4.89 4.05 0 1.93.93 3.15 3.27 3.72l1.9.46c1.39.34 1.9.93 1.9 1.77 0 1.06-.98 1.49-2.96 1.49-2.87 0-4.07-1.5-4.77-3.57l-.93-2.83C13.67 7.93 12.11 6 8.47 6 4.58 6 2 8.55 2 12.27c0 3.57 1.87 6.42 5.39 6.42 2.79 0 3.45-1.27 3.45-1.27z"/></svg>,
    fields: [{ key: 'lastfmUsername', label: 'Last.fm Username', placeholder: 'username' }],
    Component: LastfmWidget,
  },
  {
    id: 'roblox',
    name: 'Roblox',
    desc: 'Avatar, friends and followers',
    color: '#e00000',
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M5.24 3L3 18.76 18.76 21 21 5.24 5.24 3zm9.12 10.9l-3.88-.6.6-3.88 3.88.6-.6 3.88z"/></svg>,
    fields: [{ key: 'robloxUsername', label: 'Roblox Username', placeholder: 'username' }],
    Component: RobloxWidget,
  },
  {
    id: 'valorant',
    name: 'VALORANT',
    desc: 'Current rank, RR and peak rank',
    color: '#FF4655',
    note: 'Powered by Henrik\'s API (api.henrikdev.xyz)',
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 2L2 7l2 10 8 5 8-5 2-10L12 2z" stroke="currentColor" strokeWidth="1.5"/><path d="M8 9l4 7 4-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>,
    fields: [
  ...
  },
  {
    ...
    color: '#81b64c',
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M9 2v2H7v2h2v1H6v2h1l1 2H7l-1 3h1v1H5v2h14v-2h-2v-1h1l-1-3h-1l1-2h1V9h-3V8h2V6h-2V2h-2v1h-2V2H9zm3 3a1 1 0 0 1 0 2 1 1 0 0 1 0-2z"/></svg>,
    fields: [{ key: 'chessUsername', label: 'Chess.com Username', placeholder: 'username' }],
    Component: ChessWidget,
  },
  {
    id: 'tiktok',
    name: 'TikTok',
    desc: 'Followers, likes and video count',
    color: '#ff0050',
    note: 'Uses an unofficial scraper — may be unreliable',
    ...
    fields: [{ key: 'tiktokUsername', label: 'TikTok Username', placeholder: '@username' }],
    Component: TikTokWidget,
  },
  {
    id: 'discordserver',
    name: 'Discord Server',
    desc: 'Online members and live join button',
    color: '#5865F2',
    note: 'Enable Widget in Server Settings → Widget',
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.82)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#0d0505', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 18, padding: 28, width: 480, maxWidth: '95vw', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, background: `${def.color}22`, border: `1px solid ${def.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: def.color, flexShrink: 0 }}>
            {def.icon}
          </div>
          <div>
...
function ActiveWidgetCard({ widget, onRemove, onConfigure }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '16px', display: 'flex', flexDirection: 'column', gap: 14, transition: 'border-color .15s' }}
...
function AvailableTile({ def, isActive, isDisabled, onClick }) {
  ...
  return (
    ...
    >
      <div style={{ width: 40, height: 40, borderRadius: 11, background: `${def.color}20`, border: `1px solid ${def.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: def.color, flexShrink: 0 }}>
        {def.icon}
      </div>
      <div>
...
export default function WidgetsPage({ username, supabase, showToast }) {
  ...
  // Load saved widgets from DB on mount
  useEffect(() => {
    if (!username || !supabase) { setLoadingInit(false); return }
    supabase.from('users').select('widgets').eq('username', username).single()
      .then(({ data }) => {
  ...
  const saveWidgets = async () => {
    setSaving(true)
    try {
      const { error } = await supabase.from('users').update({ widgets: activeWidgets }).eq('username', username)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <style>{`
        ...
        .widget-tile-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; }
        .widget-active-grid { display:grid; grid-template-columns:repeat(2,1fr); gap:14px; }
        @media(max-width:900px){ .widget-tile-grid{grid-template-columns:repeat(2,1fr)!important;} .widget-active-grid{grid-template-columns:1fr!important;} }
        @media(max-width:600px){ .widget-tile-grid{grid-template-columns:1fr!important;} }
      `}</style>
        ...
        <div style={{ padding: '18px 22px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          ...
          {activeWidgets.length > 0 && (
            <button onClick={saveWidgets} disabled={saving}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, fontSize: 13, fontWeight: 500, cursor: saving ? 'not-allowed' : 'pointer', border: 'none', background: saving ? 'rgba(224,48,48,0.4)' : '#e03030', color: '#fff', fontFamily: 'inherit', opacity: saving ? 0.6 : 1, flexShrink: 0 }}>
